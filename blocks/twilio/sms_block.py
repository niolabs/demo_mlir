...
class Recipient(PropertyHolder):
    name = StringProperty(title='Name', default='', allow_none=True)
...
class TwilioSMS(TerminatorBlock):
    ...
    def _send_sms(self, signal):
        try:
            ...
            for rcp in self.recipients():
                Thread(target=self._broadcast_msg, args=(rcp, message, signal)).start()
    ...
    def _broadcast_msg(self, recipient, message, signal, retry=False):
        body = "%s: %s" % (recipient.name(), message)
        try:
            # Twilio sends back some useless XML. Don't care.
            response = self._client.messages.create(
                to=recipient.number(signal),
                from_=self.from_(),
                body=body
            )
